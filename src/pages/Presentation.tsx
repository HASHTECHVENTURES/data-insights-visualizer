    ...
    >
      ...
      <div
        className={cn(
          ...
          isTransitioning && direction === "forward" && "opacity-0 scale-95 translate-x-8",
          isTransitioning && direction === "backward" && "opacity-0 scale-95 -translate-x-8"
        )}
      >
      ...
      <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 flex gap-2 z-30">
        {slides.map((_, index) => (
          <button
            ...
            className={cn(
              "rounded-full transition-all hover-scale",
              index === currentSlide
                ? "bg-primary w-8 h-2"
      ...
      <div className="absolute top-8 right-8 text-xs text-muted-foreground bg-card/60 backdrop-blur-sm px-4 py-2 rounded-full border border-border z-30">
        <div className="flex flex-col gap-1">
          <span>← → or Space: Navigate</span>
          <span>F: Fullscreen</span>
          <span>Home/End: First/Last slide</span>
        </div>
      </div>
    </div>
  );
